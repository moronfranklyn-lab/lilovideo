"""前端代理与后端路由的一致性测试（**只影响开发模式**）。

生产形态已经不需要这些代理了
---------------------------
前端现在静态导出、由后端在同一端口托管，前后端**同源**，一条代理规则都不需要。
所以这个坑的危害已经从"用户会撞上"降级为"开发时会撞上"。

但仍然值得守：`scripts/launch.sh`（开发模式，前端 :3000）依然依赖 `rewrites()`，
漏配同样会出现「直连后端 200、经前端 404」，排查时一样容易误判成后端问题。
...
class TestProxyCoverage:
    def test_每一个后端前缀都有代理(self):
        ...
        assert not missing, (
            "以下后端接口前缀在开发模式下没有代理规则，经前端 :3000 会 404"
            "（直连后端却正常，极易误判；单端口模式不受影响）：\n  "
            + "\n  ".join(missing)
