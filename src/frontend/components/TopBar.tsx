...
import { CheckCircle, Circle, Loader, Edit3, AlertCircle, Square, Zap, Settings2, ChevronDown, Wallet } from 'lucide-react';
...
import { fetchSessionCost, type CostSummary } from '@/lib/workflowApi';
...
export default function TopBar({
  ...
  useEffect(() => {
    ...
    fetchSessionCost(sessionId)
      .then(sum => { if (!cancelled) setSessionCost(sum); })
      .catch(() => { if (!cancelled) setSessionCost(null); });
    return () => { cancelled = true; };
  ...
  return (
    ...
    <header
      className={clsx(
        'lilo-topbar fixed top-0 right-0 left-[var(--app-sidebar-width)] z-30 h-14 bg-white border-b border-gray-200 flex items-center px-4 min-w-0 transition-[left] duration-300',
        !hasSession && 'lilo-topbar-home'
      )}
    >
      {/* Logo & 名称 */}
      {hasSession && (
      ...
      <div className="ml-auto flex min-w-0 items-center justify-end gap-2 flex-shrink-0">
        ...
        {/* 会话成本 */}
        {hasSession && sessionCost && (
          <div
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium bg-gray-50 text-gray-600 ring-1 ring-gray-100"
            title="本会话累计成本（含各阶段模型调用，未计入积分扣减）"
          >
            <Wallet className="w-3.5 h-3.5 text-gray-400" />
            <span className="tabular-nums">¥{sessionCost.total.toFixed(2)}</span>
            {sessionCost.has_unknown && (
              <span className="text-[10px] text-gray-400">部分成本未知</span>
            )}
          </div>
        )}
