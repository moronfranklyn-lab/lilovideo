...
- 支持逐项并发生成、实时预览、重新生成
- 支持先出样片（preview）再批量生成：视频按时长计费，先花钱验证质量比事后重做便宜
"""
...
from models.cost_store import submit_with_cost_context

logger = logging.getLogger(__name__)

# 样片时长（秒）。选取依据：短视频平台常见的最短可评估长度，
# 同时把成本压到接近最低。Seedance 2.0 约 1 元/秒，因此样片约 5 元。
...
class VideoDirectorAgent(AgentInterface):
    ...
    async def process(self, input_data: Any, intervention: Optional[Dict] = None) -> Dict:
                ...
                def regen_run():
                    ...
                    with ThreadPoolExecutor(max_workers=concurrency) as executor:
                        ...
                        for seg_id in regen_ids:
                            ...
                            fut = submit_with_cost_context(executor,
        ...
        # ═══ 介入：先出样片，确认质量后再批量生成 ═══
        #
        # 为什么要这个模式：视频按时长计费（Seedance 2.0 约 1 元/秒），
        # 整片批量生成后才发现风格不对，返工成本极高。而参考图阶段成本很低。
        # 因此先用 1 个片段、PREVIEW_CLIP_SECONDS 秒验证效果，确认后再生成其余。
        preview_only = bool(intervention and intervention.get("generate_preview"))
        if preview_only:
            if not segments:
                raise Exception("未找到可预览的分镜片段")
            first = segments[0]
            logger.info(
                "[VideoAgent] 样片模式：仅生成首片段 %s，时长 %ss",
                first.get("segment_id"), PREVIEW_CLIP_SECONDS,
            )
            segments_to_run = [first]
        else:
            segments_to_run = segments

        def run():
            tasks = []
            for seg_index, seg in enumerate(segments_to_run):
                ...
                if video_generation_mode == "reference":
                    ...
                    img_path = self._get_reference_image(sid, seg_id, scene_map)
                # 样片模式只改时长；索引仍取原始列表，否则首尾帧模式的尾帧会取错
                if preview_only:
                    duration = PREVIEW_CLIP_SECONDS
                else:
                    duration = seg.get("total_duration", 10)
                last_img_path = self._get_next_reference_image(sid, seg_index, segments, scene_map)
                if video_generation_mode == "start_end_frame" and not last_img_path:
        ...
        result = self._build_payload(sid, segments, video_clips)

        # 标记样片状态，让前端知道还有多少片段没生成、要不要显示「继续生成全片」
        if preview_only:
            result["preview"] = {
                "preview_only": True,
                "preview_seconds": PREVIEW_CLIP_SECONDS,
                "generated": len(segments_to_run),
                "total": len(segments),
                "remaining": max(len(segments) - len(segments_to_run), 0),
            }
            self._report_progress(
                "视频生成",
                f"样片已完成（{PREVIEW_CLIP_SECONDS} 秒），确认后可生成其余 {max(len(segments) - 1, 0)} 个片段",
                100,
            )
        else:
            result["preview"] = {
