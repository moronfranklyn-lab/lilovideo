...
/**
 * LaserFlow —— 激光能量流背景组件（THREE.js 实现，无第三方额外依赖）。
 *
 * 由 Ethan 提供源码（ReactBits 风格），整理为 TypeScript 组件落地：
 * 全屏 quad + RawShaderMaterial，光束/卷须/雾效 shader，
 * 深色背景下用 mix-blend-mode: screen 融入页面。
 */
...
export const LaserFlow = ({
  ...
  useEffect(() => {
    ...
    const applyBackgroundMode = (value: string) => {
      ...
      mount.style.backgroundColor = 'transparent'; // 容器背景透明：让页面光晕透出，避免黑底盖住毛玻璃
