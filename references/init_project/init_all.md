...
从仓库根目录进入 `lilovideo/lilovideo` 后执行：

```bash
# macOS / Linux
cd lilovideo/lilovideo
chmod +x install.sh
./install.sh
```
...
安装脚本会完成后端依赖、`config.yaml` 初始化、前端依赖安装和前端 build。安装完成后，提醒用户编辑 `src/backend/config.yaml` 填入 API Key，或启动前端后通过侧边栏底部「设置」页面配置。
