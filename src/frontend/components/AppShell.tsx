...
import { CheckCircle2, ChevronLeft, ChevronRight, Clapperboard, Clock, Hexagon, Home, Loader2, Repeat2, Settings, Trash2, UserRound, Coins } from 'lucide-react';
import clsx from 'clsx';
import { useEffect, useRef, useState, type CSSProperties } from 'react';
import { clearTempCache, fetchPipelineTasks, fetchSandboxTasks, fetchSessions, fetchCreditsAccount, fetchCreditsLedger, type PipelineTask, type SandboxTask, type CreditsAccount, type CreditsLedgerItem } from '@/lib/workflowApi';
...
const COMPLETED_TASK_DISMISS_KEY = 'lilovideo.dismissed-completed-tasks';
const SIDEBAR_OPEN_KEY = 'lilovideo.sidebar-open';
...
function TaskPanel({
  ...
  return (
    <section className="lilo-task-panel h-[20vh] min-h-32 border-t border-gray-100 p-3">
            ...
            {tasks.map(task => {
              ...
              return (
                <button
                  ...
                  className={clsx(
                    'lilo-task-item w-full rounded-lg border px-2.5 py-2 text-left transition-colors',
                    active ? 'border-violet-100 bg-violet-50/60' : 'border-gray-100 bg-white hover:border-violet-200 hover:bg-violet-50/40'
                  )}
                >
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    ...
                    <div className="h-1 min-w-10 flex-1 overflow-hidden rounded-full bg-gray-100">
                      <div className="h-full rounded-full bg-gradient-to-r from-violet-500 to-fuchsia-400" style={{ width: `${task.progress || 0}%` }} />
                    </div>
                  </div>
                </button>
              );
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const [account, setAccount] = useState<CreditsAccount | null>(null);
  const [ledger, setLedger] = useState<CreditsLedgerItem[]>([]);
  ...
  const refreshCredits = async () => {
    ...
    setCreditsError('');
    try {
      const [acc, led] = await Promise.all([fetchCreditsAccount(), fetchCreditsLedger(50)]);
      setAccount(acc);
      setLedger(led.items);
    } catch (e: any) {
      setCreditsError(e?.message || '加载积分失败');
    } finally {
  ...
  useEffect(() => {
    ...
    if (creditsOpen) document.addEventListener('mousedown', handler);
  ...
  return (
    <div
      className="lilo-shell min-h-screen bg-gray-50 text-gray-800"
      style={{ '--app-sidebar-width': open ? '15.5rem' : '0px' } as CSSProperties}
    >
      <aside
        className={clsx(
          'lilo-sidebar fixed inset-y-0 left-0 z-40 border-r border-gray-200 bg-white shadow-sm transition-all duration-300',
          !open && 'border-r-0'
        )}
      ...
      >
        <div className={clsx('flex h-full flex-col overflow-hidden transition-opacity duration-200', open ? 'opacity-100' : 'opacity-0')}>
          <div className="lilo-sidebar-brand flex h-16 items-center px-4 border-b border-gray-100">
            <div className="flex items-center gap-2 min-w-0">
              <img src={BRAND_ICON.mark} alt={`${BRAND.name} Logo`} className="h-9 w-9 rounded-xl object-contain" />
              <span className="text-base font-semibold text-gray-900 truncate">{BRAND.name}</span>
            </div>
          </div>
          <nav className="lilo-sidebar-nav min-h-0 flex-1 overflow-y-auto p-3 space-y-1">
            {NAV_ITEMS.map(item => {
              ...
              return (
                <Link
                  ...
                  href={item.href}
                  className={clsx(
                    'lilo-sidebar-link flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors',
                    active
                      ? 'bg-violet-50 text-violet-700'
                      : 'text-gray-500 hover:bg-violet-50/60 hover:text-violet-600'
                  )}
                >
          ...
          <div className="lilo-sidebar-footer relative border-t border-gray-100 p-3">
            <div className="mb-1 relative" ref={creditsRef}>
              <button
                type="button"
                onClick={() => { setCreditsOpen(v => !v); if (!account) refreshCredits(); }}
                className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors bg-violet-50 text-violet-700 hover:bg-violet-100"
              >
                <Coins className="h-4 w-4 flex-shrink-0" />
                <span className="truncate">
                  积分 <span className="tabular-nums font-semibold">{account ? account.balance : (creditsLoading ? '…' : '—')}</span>
                </span>
                {account && (
                  <span className="ml-auto text-[10px] text-violet-600/70">
                    ≈¥{account.balance_value_yuan.toFixed(1)}
                  </span>
                )}
              </button>
              <CreditsPanel
                ...
                panelClassName="absolute bottom-full mb-2 left-0"
              />
            </div>
            {(() => {
              ...
              return (
                <>
                  {settingsMenuOpen && (
                    <div className="lilo-settings-menu absolute bottom-[62px] left-3 right-3 rounded-xl border border-gray-200 bg-white p-2 shadow-lg">
                      <button
                        type="button"
                        onClick={() => {
                          setSettingsMenuOpen(false);
                        ...
                        className="flex h-10 w-full items-center gap-2 rounded-lg px-3 text-left text-sm font-medium text-gray-600 hover:bg-violet-50 hover:text-violet-600"
                      >
      ...
      {open && (
        <button
          ...
          className="lilo-sidebar-toggle fixed top-1/2 z-50 h-14 w-7 -translate-y-1/2 rounded-r-xl border border-l-0 border-gray-200 bg-white text-gray-400 shadow-sm hover:w-9 hover:text-gray-900 hover:border-gray-300 hover:bg-white flex items-center justify-center transition-all"
      ...
      {!open && (
        <button
          ...
          className="fixed left-0 top-1/2 z-50 h-14 w-7 -translate-y-1/2 rounded-r-xl border border-l-0 border-gray-200 bg-white text-gray-400 shadow-sm hover:w-9 hover:text-violet-600 hover:border-violet-200 hover:bg-violet-50 flex items-center justify-center transition-all"
          title="打开侧边栏"
          aria-label="打开侧边栏"
        >
      ...
      <main
        className="lilo-main min-h-screen min-w-0 overflow-x-hidden transition-[margin] duration-300"
