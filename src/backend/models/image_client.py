...
class ImageClient:
    ...
    def generate_image(self,
        ...
        # 记账：图片按**张**计费（不是按次），所以必须按实际返回的路径数记账。
        # 放在这个唯一的出口处，三种后端（Seedream / DashScope / GPT）共用一处逻辑，
        # 避免各自漏记。生成失败时不产生成本，故只对成功返回的图计数。
        if generated_local_paths:
            report_usage(Usage(model_id=model, images=len(generated_local_paths)))
            logger.info(
                "ImageClient: 计费 %d 张 model=%s", len(generated_local_paths), model
            )
