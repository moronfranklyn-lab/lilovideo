...
export default function VideoStage({ state, sessionId, onConfirm, onIntervene, onRegenerate, onUpdateArtifact, onSaveSelections, showConfirm, isRunning, referenceArtifact, hasPendingItems, hasNextStageStarted, scriptArtifact }: StageViewProps) {
  ...
  const doneClips = clips.filter(c => c.selected || (c.versions && c.versions.length > 0));
  const previewDone =
    previewRequested &&
    state.status !== 'running' &&
    doneClips.length === 1 &&
    doneClips.length < clips.length;
  const remainingCount = clips.length - doneClips.length;
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex-1 min-w-0 overflow-y-auto p-4 sm:p-6">
        ...
        {/* ═══ 样片模式：先出样片再批量生成 ═══ */}
        {!previewRequested && !previewDone && state.status !== 'running' && doneClips.length === 0 && hasClips && (
          <div className="mb-4 flex flex-wrap items-center gap-3 px-4 py-3 rounded-xl bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200">
            <div className="flex-1 min-w-[200px]">
              <div className="text-sm font-medium text-amber-800">先出样片，确认质量再批量生成</div>
              <div className="text-xs text-amber-700/80 mt-0.5">
                视频按秒计费，先用首个片段（5 秒）验证画面与风格，避免整片生成后返工
              </div>
            </div>
            <button
              onClick={() => { setPreviewRequested(true); onIntervene({ generate_preview: true }); }}
              disabled={isRunning}
              className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium bg-white text-orange-600 border border-orange-300 hover:bg-orange-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
        ...
        {previewRequested && state.status === 'running' && (
          <div className="mb-4 flex items-center gap-2 px-4 py-3 rounded-xl bg-amber-50 border border-amber-200 text-sm text-amber-700">
            <Loader className="w-4 h-4 animate-spin" />
            正在生成样片（首个片段 5 秒）…
          </div>
        )}
        ...
        {previewDone && (
          <div className="mb-4 flex flex-wrap items-center gap-3 px-4 py-3 rounded-xl bg-emerald-50 border border-emerald-200">
            <div className="flex-1 min-w-[200px]">
              <div className="text-sm font-medium text-emerald-800">样片已完成（5 秒）</div>
              <div className="text-xs text-emerald-700/80 mt-0.5">
                还剩 {remainingCount} 个片段未生成。确认效果后点击「生成其余片段」补齐全片，再「确认并继续」进入后期剪辑
              </div>
            </div>
            <button
              onClick={() => { setPreviewRequested(false); onRegenerate(); }}
              disabled={isRunning}
              className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium bg-emerald-600 text-white hover:bg-emerald-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <Film className="w-4 h-4" />
              生成其余 {remainingCount} 个片段
            </button>
          </div>
        )}
