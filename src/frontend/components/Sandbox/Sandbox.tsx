...
import { Sparkles, Image, Video, MessageSquare, Zap, Loader2, Copy, Check, Trash2, X, FolderOpen, Upload, Globe, Hexagon, RefreshCw } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {output.images.map((img, i) => (
            <a key={i} href={toMediaUrl(img)} target="_blank" rel="noopener noreferrer" className="group block rounded-xl border border-gray-200 bg-white overflow-hidden">
              ...
              <div className="px-3 py-2 text-xs text-gray-500 group-hover:text-violet-600 border-t border-gray-100">查看图片</div>
            </a>
          ))}
        </div>
      )}
      {output.video_path && (
        <div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
          ...
          <div className="px-3 py-2 border-t border-gray-100">
            <a href={toMediaUrl(output.video_path)} target="_blank" rel="noopener noreferrer" className="text-sm text-violet-600 hover:underline">
              查看视频
            </a>
          </div>
        </div>
      ...
      <div className="flex gap-2 mb-2">
        <button
          ...
          className={`text-xs px-3 py-1.5 rounded-lg transition-colors ${
            inputMode === 'url' ? 'bg-violet-100 text-violet-700' : 'text-gray-500 hover:bg-gray-100'
          }`}
        >
          URL 地址
        </button>
        <button
          ...
          className={`text-xs px-3 py-1.5 rounded-lg transition-colors ${
            inputMode === 'file' ? 'bg-violet-100 text-violet-700' : 'text-gray-500 hover:bg-gray-100'
          }`}
        >
          本地上传
        </button>
      ...
      {inputMode === 'url' && (
        <div className="space-y-2">
          <input
            ...
            className="w-full px-4 py-2.5 border border-gray-200 rounded-lg focus:ring-2 focus:ring-violet-500 focus:border-transparent outline-none"
          />
          {value && isUrl && (
          ...
          {value && !isUrl ? (
            ...
            <div
              ...
              className={`border-2 border-dashed rounded-lg p-8 text-center cursor-pointer transition-colors ${
                isDragging ? 'border-violet-500 bg-violet-50' : 'border-gray-300 hover:border-gray-400'
              }`}
            >
              <input
                ref={fileInputRef}
              ...
              <p className="text-sm text-gray-500">
                拖拽图片到此处，或 <span className="text-violet-600">点击选择文件</span>
              </p>
...
export default function SandboxPage() {
  ...
  const [lastFailure, setLastFailure] = useState<{ tool: ToolType; prompt: string; imageUrl: string; model: string; webSearch: boolean } | null>(null);

  const runSandbox = async (tool: ToolType, p: string, img: string, model: string, ws: boolean) => {
    if (!model) return;
    setLoading(true);
    setResult(null);
    setCurrentOutput(null);
    ...
    try {
      ...
      if (data.success) {
        if (tool === 't2i' || tool === 'i2i' || tool === 'video') {
          const output = tool === 'video'
      ...
      setLastFailure({ tool, prompt: p, imageUrl: img, model, webSearch: ws });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = () => {
    if (!canSubmit()) return;
    runSandbox(activeTool, prompt, imageUrl, selectedModel, webSearch);
  };

  const handleRetry = () => {
    if (!lastFailure) return;
    runSandbox(lastFailure.tool, lastFailure.prompt, lastFailure.imageUrl, lastFailure.model, lastFailure.webSearch);
  };
  ...
  return (
    <div className="min-h-screen sandbox-page-bg">
      <BrandHeader />
        ...
        <>
            <div className="mb-8 text-center">
              <div className="inline-flex items-center gap-2 mb-3">
                <Hexagon className="w-7 h-7 text-violet-500" />
                ...
                <button
                  ...
                  className={`p-4 rounded-xl border-2 transition-all text-center ${
                    activeTool === tool.id
                      ...
                      : 'border-violet-100 bg-white/80 text-gray-600 hover:border-violet-300 hover:shadow-sm'
                  }`}
                >
            ...
            {/* 输入区域 */}
            <div className="bg-white/75 backdrop-blur-md rounded-2xl shadow-md shadow-violet-100/40 border border-violet-100 p-6 mb-6">
              {/* 模型选择 */}
              <div className="mb-4">
              ...
              <div className="mb-4">
                ...
                <textarea
                  ...
                  rows={4}
                  className="w-full px-4 py-3 border border-violet-200/70 bg-white/80 rounded-xl focus:ring-2 focus:ring-violet-400/60 focus:border-transparent outline-none resize-none backdrop-blur-sm transition-colors"
                />
              </div>
              ...
              <button
                ...
                className="w-full py-3 px-6 bg-gradient-to-r from-violet-600 to-purple-500 text-white font-medium rounded-xl hover:from-violet-700 hover:to-purple-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2 shadow-md shadow-violet-200/50 hover:shadow-lg hover:shadow-violet-200/60 active:scale-[0.99]"
              >
                {loading ? (
                  <>
            ...
            {/* 结果展示 */}
            {(currentOutput || error) && (
              <div className={`rounded-2xl border p-6 ${error ? 'bg-red-50 border-red-200' : 'bg-white/90 border-violet-200'}`}>
                <div className="flex items-center justify-between mb-3">
                  <h3 className={`font-medium ${error ? 'text-red-700' : 'text-violet-700'}`}>
                    {error ? '错误' : '生成结果'}
                  </h3>
                  ...
                  {error && lastFailure && (
                    <button
                      onClick={handleRetry}
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white border border-red-200 text-red-600 text-xs font-medium hover:bg-red-50 transition-colors active:scale-95"
                    >
                  ...
                  {!error && currentOutput && (
                    <button onClick={copyResult} className="p-2 rounded-lg hover:bg-white/50 transition-colors" title="复制结果">
                      {copied ? <Check className="w-4 h-4 text-violet-600" /> : <Copy className="w-4 h-4 text-gray-500" />}
                    </button>
                  )}
                  </div>
                </div>
          ...
          <section className="mt-10">
            ...
            {history.filter(record => record.tool === activeTool).length === 0 ? (
              <div className="h-32 rounded-xl border border-dashed border-violet-200 bg-white/60 flex items-center justify-center text-sm text-gray-400">
                暂无历史记录
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {history.filter(record => record.tool === activeTool).map(record => (
                  <div
                    ...
                    className={`bg-white rounded-xl border border-gray-200 p-4 hover:border-violet-300 hover:shadow-md hover:shadow-violet-100/40 transition-all ${manageMode ? '' : 'cursor-pointer'}`}
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1.5 flex flex-wrap items-center gap-2">
                          <span className="text-[11px] bg-violet-50 text-violet-600 px-1.5 py-0.5 rounded">{record.model}</span>
                          <span className="text-[11px] text-gray-400">{formatDate(record.created_at)}</span>
                        </div>
                      </div>
                      {manageMode && (
                        <button
