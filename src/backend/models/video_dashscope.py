...
class DashscopeVideoClient:
    ...
    def generate_video(
        ...
        # 记账：通义视频按**秒**计费，秒数就是请求的 duration。
        #
        # 这里不需要从响应里读用量——这是 per_second 相对 per_token 的好处：
        # 成本在提交请求的那一刻就确定了，不依赖响应体。
        #
        # 但必须把**分辨率和是否带音轨**一起报上去：同一模型
        # 720P 无声 ¥0.15/秒、1080P 带音 ¥0.50/秒，差 3.3 倍。
        # 只报秒数会把 1080P 记成 720P 的价，少记 40%~67% 且不报错。
        report_usage(Usage(
            model_id=model,
