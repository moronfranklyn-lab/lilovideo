...
> **产物存放目录**：`src/backend/code/result/`
> - `script/` - 剧本产物
> - `image/` - 图片产物（角色、场景、参考图）
> - `video/` - 视频产物
> - `task/<task_id>/` - Pipeline 产物（文艺短视频、动作迁移、数字人口播）
>
> **Pipeline 元数据目录**：`src/backend/code/data/tasks/<task_id>.json`
...
3. 检查 API Key 配置 → 读取 `src/backend/config.yaml`，确认所需 API Key 已配置
...
```bash
# 读取 config.yaml 检查配置
sed -n '1,120p' src/backend/config.yaml
...
1. 告知缺少哪个平台的 API Key
2. 提供获取方式
3. 配置位置（`src/backend/config.yaml` 文件，或前端侧边栏底部「设置」页面）
4. 等待用户配置完成后才能继续
