...
class VideoClient:
    ...
    def _normalize_seedance_resolution(resolution: Optional[str]) -> str:
        """把分辨率规整成 Seedance 认识的小写写法。

        480p 是**草稿档**：Seedance 按 tokens 计费，而 tokens 与像素数成正比，
        所以 480p 的 tokens 约为 720p 的 45%（¥0.50/秒 → ¥0.23/秒），
        适合"先出草稿看构图、再用高档重出关键镜头"的组合打法。

        认不出的值退到 720p（比退到 480p 安全：宁可多花钱也不要静默降质）。
        """
        value = (resolution or "720p").strip().lower()
        return value if value in {"480p", "720p", "1080p"} else "720p"
