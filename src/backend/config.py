...
DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    "models": {
        # 这些默认值已对齐**本项目账号实测可用**的模型。
        # 上游戏模板里的默认值（qwen3.5-plus / seedream-5.0 / wan2.7）在本账号
        # 不可用，若 config.yaml 丢失而回退到默认值，会静默调到不存在的模型。
        "llm": "deepseek-v4-1-flash-260910",
        "vlm": "qwen3.7-plus",
        "image_it2i": "doubao-seedream-4-0-250828",
        "image_t2i": "doubao-seedream-4-0-250828",
        # 视频默认走方舟 Seedance 2.0；通义侧的可选档位见《积分换算表.md》
        "video": "doubao-seedance-2-0-260128",
        "video_first_frame": "doubao-seedance-2-0-260128",
        "video_start_end": "doubao-seedance-2-0-260128",
        "video_reference": "doubao-seedance-2-0-260128",
    },
    "generation": {
        "style": "realistic",
        "video_ratio": "16:9",
...
# ── 部署安全：环境变量优先 ─────────────────────────────────────────────
#
# 线上部署时密钥只放在服务端环境变量里，不落可写文件。这样即使 config.yaml
# 被改写，也拿不到有效密钥。本地开发不设环境变量时行为完全不变。
#
# 详见 deployment.py 的设计说明。
from deployment import apply_env_overrides, strip_env_secrets as _strip_env_secrets  # noqa: E402

CONFIG_VALUES, _ENV_SECRET_PATHS = apply_env_overrides(CONFIG_VALUES)


class Config:
    CONFIG = CONFIG_VALUES

    # 运行模式：按监听地址判定，local 允许网页改配置，public 禁止。
    # 见 deployment.py 的 detect_mode。
    from deployment import detect_mode as _detect_mode  # noqa: E402

    MODE = _detect_mode(_get(CONFIG, "server.host"))
    IS_PUBLIC = MODE == "public"
    # 本次启动中由环境变量接管的密钥配置点，用于启动日志提示
    ENV_SECRET_PATHS = list(_ENV_SECRET_PATHS)
    ...
    def update_config(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        merged = merge_config_update(cls.CONFIG, values)
        # 落盘前先剥离环境变量提供的密钥，避免真实密钥被写进 config.yaml
        # （否则「密钥只存在环境变量」这条防线就失效了）。
        to_disk = _strip_env_secrets(merged)
        save_config(to_disk)
        # 内存中保留完整配置：文件值 + 环境变量覆盖
        clean, _ = apply_env_overrides(to_disk)
        cls.CONFIG = clean
