...
from models import cost_store, credits_store as credit_store
from models.credit_estimator import estimate_stage_credits
from models.credits_store import credits_for_cost
...
# 阶段中文名。放在这里而不是 api/ 下，是因为 orchestrator 写积分流水时要用它做
# 「消费原因」，而 core/ 不该反向依赖 api/。
# 注意 api/routers/stages.py 与 api/services/project_helpers.py 里也各有一份，
# 三处必须一致——改名字时一起改。
...
class WorkflowEngine:
    ...
    # ──────────── 成本记账 ────────────
    def _log_stage_cost(self, session_id: str, stage: str) -> None:
        """阶段结束后记一行成本日志。

        这一行的作用不是给用户看，是**让"漏记"变得可见**。

        成本记账依赖作用域包裹（见 `execute_stage`）。如果有人以后新增了一个
        线程池却忘了用 `submit_with_cost_context`，那段花费就会静默丢失——
        不报错、不写警告，只是账少了。这种 bug 靠读代码很难发现，
        但"本阶段 0 笔记录"这句话在日志里会立刻扎眼。
        """
        try:
            summary = cost_store.get_summary(session_id)
        except Exception as exc:  # noqa: BLE001 - 记账日志不得影响主流程
            logger.warning("[cost] 读取阶段成本失败 session=%s: %s", session_id, exc)
            return

        stage_amount = summary.get("by_stage", {}).get(stage, 0.0)
        logger.info(
            "[cost] 阶段 %s 结束：本阶段 ¥%.4f，会话累计 ¥%.4f（%d 笔）%s",
            stage,
            stage_amount,
            summary.get("total", 0.0),
            summary.get("call_count", 0),
            "  ⚠️ 本阶段没有任何成本记录，检查是否有线程池漏传计费上下文"
            if stage_amount == 0 else "",
        )
    ...
    async def execute_stage(self,
        ...
        # 预扣积分：阶段开始前先占住一笔，结束时按**实际用量**结算。
        #
        # 为什么必须预扣而不是跑完再扣：一阶段可能跑几分钟、烧掉上百积分，
        # 跑完才发现余额不够就只能产生缺口。预扣把"付不起"挡在阶段之前。
        #
        # ref 用 "{session_id}:{stage}"，所以重试、连点、断点续跑都不会重复扣费。
        reservation_ref = f"{state.session_id}:{stage.value}"
        reserved_credits = self._reserve_stage_credits(
            state, stage, input_data, reservation_ref
        )

        try:
            # 开一个计费作用域：这一阶段里所有模型调用都会自动记到
            # 该会话的该阶段上，客户端不需要知道 session_id。
            #
            # 为什么必须在这里包住整个 agent.process：模型调用发生在阶段内部
            # 6 个线程池里，客户端只拿得到一个"当前会话+阶段"的上下文。
            # 作用域漏开的话成本会**静默丢失**，所以下面还补了一条收尾日志。
            with cost_store.cost_scope(state.session_id, stage.value):
                result = await agent.process(input_data, intervention=intervention)
            self._log_stage_cost(state.session_id, stage.value)
            self._settle_stage_credits(state.session_id, stage.value, reservation_ref)
            ...
            with self._state_lock:
                ...
                self.save_session_to_disk(state.session_id)
            # 失败也要**结算到实际用量**，而不是全额退回。
            #
            # 理由：阶段中途失败时，前面已经跑掉的模型调用是**真花了钱**的
            # （比如 6 张参考图出了 3 张）。全额退回等于我们自己承担那部分成本，
            # 而且账面成本（cost_store）与积分（这里）会对不上。
            # 若确实一分钱没花，实际用量为 0，结算就等于全额退回——
            # 所以这条路径天然覆盖"全额退"的情况。
            self._settle_stage_credits(state.session_id, stage.value, reservation_ref, "已取消")
            raise
            ...
            with self._state_lock:
                ...
                # 确保保存错误状态
                self.save_session_to_disk(state.session_id)
            # 失败同样结算到实际用量（见 CancelledError 分支的说明）
            self._settle_stage_credits(state.session_id, stage.value, reservation_ref, "执行失败")
            raise
        finally:
    ...
    # ──────────── 积分预扣 / 结算 ────────────
    def _reserve_stage_credits(
        self, state: WorkflowState, stage: WorkflowStage, input_data: Dict, ref: str
    ) -> int:
        """阶段开始前预扣积分。返回预扣额；余额不足时抛出。

        **余额不足会直接中断阶段**——这是刻意的：让用户在花掉 API 额度之前
        就被拦住，而不是跑完几分钟再发现付不起。

        预扣失败（账本异常等）不阻断生成流程，只记日志：
        积分记账不该让功能不可用。
        """
        try:
            segment_count, model, resolution = self._estimate_inputs(state, stage, input_data)
            credits = estimate_stage_credits(
                stage.value,
            ...
            if credits <= 0:
                return 0
            credit_store.reserve(
                credit_store.LOCAL_USER, credits, f"{STAGE_LABELS.get(stage.value, stage.value)}",
                ref=ref, session_id=state.session_id, stage=stage.value,
            )
            return credits
        except credit_store.InsufficientCredits:
            # 余额不足是**用户可理解、可处理**的错误，必须冒泡到界面
            raise
        except Exception as exc:  # noqa: BLE001 - 记账异常不得让生成不可用
            logger.warning("[credits] 预扣失败，本次不做预扣（生成继续）：%s", exc)
            return 0

    def _estimate_inputs(
        self, state: WorkflowState, stage: WorkflowStage, input_data: Dict
    ) -> tuple[int, Optional[str], Optional[str]]:
        """给预估器准备入参：待生成镜头数、所选视频模型与分辨率。

        只有视频阶段用得上，且必须尽量取准——它占一条成片成本的约 80%，
        估错会直接导致"该放行的挡在门外"或"放进来却付不起"。
        """
        if stage != WorkflowStage.VIDEO_GENERATION:
            return 0, None, None
        ...
        segments: list = []
        try:
            storyboard = (state.artifacts or {}).get("storyboard") or {}
            for episode in storyboard.get("episodes", []) or []:
                segments.extend(episode.get("segments", []) or [])
            if not segments:
                segments = storyboard.get("segments", []) or []
        except Exception as exc:  # noqa: BLE001
            logger.warning("[credits] 取分镜失败，视频阶段预估退回兜底值：%s", exc)

        model = (
            input_data.get("video_model")
            or (state.meta or {}).get("video_model")
        )
        resolution = (
            input_data.get("video_resolution")
            or (state.meta or {}).get("video_resolution")
            or "720P"
        )
        return len(segments), model, resolution

    def _settle_stage_credits(
        self, session_id: str, stage: str, ref: str, note: str = ""
    ) -> None:
        """阶段结束（无论成功失败）按**实际成本**结算积分。

        实际成本来自 `cost_store` 记的真实金额——那是唯一可信的来源。
        结算会把预扣调整到实际值：多了退、少了补。

        这里绝不让异常冒泡：阶段已经跑完，记账失败不该把它变成失败。
        但会记 ERROR 并把预扣留在 pending，便于事后用
        `credits_store.pending_reservations()` 查出来处理。
        """
        try:
            actual_cost = cost_store.get_summary(session_id).get("by_stage", {}).get(stage, 0.0)
            actual_credits = credits_for_cost(actual_cost)
            credit_store.settle(ref, actual_credits)
            logger.info(
                "[credits] 阶段 %s 结算：成本 ¥%.4f → %d 积分%s",
                stage, actual_cost, actual_credits, f"（{note}）" if note else "",
            )
        except Exception as exc:  # noqa: BLE001
            logger.error(
                "[credits] 阶段 %s 积分结算失败（预扣仍为 pending，ref=%s）：%s",
                stage, ref, exc,
            )

    def _release_stage_credits(self, session_id: str, stage: str, ref: str, note: str) -> None:
        """把预扣全额退回。仅在**确认没有产生任何成本**时使用。"""
        try:
            credit_store.release(ref, reason=note)
            logger.info("[credits] 阶段 %s 预扣已退回（%s）", stage, note)
        except Exception as exc:  # noqa: BLE001
            logger.error("[credits] 阶段 %s 退回预扣失败（ref=%s）：%s", stage, ref, exc)
