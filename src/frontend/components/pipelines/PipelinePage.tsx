  ...
  return (
    <label className="flex flex-col gap-1.5 min-w-0">
      ...
      <select
        ...
        className="h-10 rounded-lg border border-gray-200 bg-white px-3 text-sm text-gray-700 outline-none focus:border-violet-300"
      >
        {groups.map(group => (
    ...
    <label className="flex flex-col gap-1.5 min-w-0">
      ...
      <div className="flex gap-2">
        <input
          ...
          placeholder={placeholder}
          className="h-10 min-w-0 flex-1 rounded-lg border border-gray-200 bg-white px-3 text-sm text-gray-700 outline-none focus:border-violet-300"
        />
...
function FinalVideoResult({ item }: { item?: PipelineArtifact }) {
  return (
    <section className="mb-4 rounded-xl border border-violet-100 bg-violet-50/40 p-3">
      ...
      {item ? (
        <div className="overflow-hidden rounded-lg border border-violet-100 bg-black">
          <video src={assetHref(item.path)} controls className="w-full max-h-72 object-contain" />
          <div className="px-3 py-2 flex items-center justify-between bg-white/90 border-t border-violet-100/70">
            <span className="text-xs text-gray-500 truncate">{item.name || '成片'}</span>
            <a href={assetHref(item.path)} target="_blank" rel="noopener noreferrer" className="text-xs font-medium text-violet-600 hover:underline">
              查看 / 下载
            </a>
          </div>
        </div>
      ) : (
        <div className="h-36 rounded-lg border border-dashed border-violet-200 bg-white/60 flex items-center justify-center text-sm text-gray-400">
          成片生成后会展示在这里，可直接播放和下载
        </div>
      )}
    </section>
  );
...
function TaskResult({ task, onRetry }: { task: PipelineTask | null; onRetry?: () => void }) {
  const progress = Math.max(0, Math.min(100, task?.progress || 0));

  if (!task) {
    return (
      <div className="lilo-panel-card p-5 h-full">
        ...
        <div className="mb-4 h-1.5 rounded-full bg-violet-100/60 overflow-hidden">
          <div className="h-full w-0 rounded-full bg-violet-500" />
        </div>
        <div className="h-48 rounded-xl border border-dashed border-violet-200 bg-violet-50/40 flex items-center justify-center text-sm text-gray-400">
          任务启动后，进度和结果会显示在这里
        </div>
      </div>
    );
  ...
  return (
    <div className="lilo-panel-card p-5 h-full">
      ...
      <div className="mb-4 h-1.5 rounded-full bg-violet-100/60 overflow-hidden">
        <div
          className={clsx('h-full rounded-full transition-all', task.status === 'failed' ? 'bg-red-500' : 'bg-violet-500')}
      ...
      {task.status === 'failed' && (
        <div className="mb-4 rounded-xl border border-red-200 bg-red-50/70 p-3">
          ...
          <p className="text-xs leading-5 text-red-600/90">
            {task.error || task.message || '生成过程中出现异常，请检查素材和配置后重试'}
          </p>
          {onRetry && (
            <button
              onClick={onRetry}
              className="mt-2 flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white border border-red-200 text-red-600 text-xs font-medium hover:bg-red-100 transition-colors active:scale-95"
            >
      ...
      {mediaArtifacts.length > 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {mediaArtifacts.map((item, index) => (
              <div
                ...
                className="h-44 rounded-xl border border-violet-100 bg-violet-50/40 overflow-hidden min-w-0"
              >
                <div className="h-8 px-3 border-b border-violet-100/70 bg-white/80 flex items-center gap-2">
                  {item.kind === 'video' && <Video className="w-3.5 h-3.5 text-violet-500" />}
        ...
        <div className="h-48 rounded-xl bg-violet-50/40 border border-dashed border-violet-200 flex items-center justify-center text-sm text-gray-400">
          结果生成后显示
        </div>
      )}
    </div>
  ...
  return (
    <button
      ...
      className={clsx(
        ...
        selected ? 'border-violet-400 ring-2 ring-violet-100' : 'border-gray-200 hover:border-violet-300 hover:shadow-sm'
      )}
    ...
    >
      ...
      <div className="mt-1 max-w-36 truncate text-[11px] font-medium text-gray-500 group-hover:text-violet-600">
        {template.label}
      </div>
    </button>
  );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {tasks.map(task => (
          <div
            ...
            className={clsx(
              'group text-left p-4 bg-white rounded-xl border hover:border-violet-300 hover:shadow-sm transition-all',
              manageMode ? 'cursor-default' : 'cursor-pointer',
              activeTaskId === task.task_id ? 'border-violet-300 ring-2 ring-violet-50' : 'border-gray-200'
            )}
          >
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="text-sm font-medium text-gray-700 group-hover:text-violet-600 transition-colors truncate">
                ...
                <div className="mt-2 h-1 rounded-full bg-gray-100 overflow-hidden">
                  <div className="h-full bg-violet-500 rounded-full" style={{ width: `${task.progress || 0}%` }} />
                </div>
              </div>
              {manageMode ? (
                <button
                ...
                <ArrowRight className="w-4 h-4 text-gray-300 group-hover:text-violet-400 flex-shrink-0 mt-0.5" />
              )}
            </div>
          </div>
        ))}
...
export default function PipelinePage({ pipeline, title, subtitle }: PipelinePageProps) {
  ...
  return (
    <div className="min-h-screen lilo-pipeline-canvas overflow-y-auto">
      <BrandHeader />

      <div className="w-full max-w-6xl mx-auto px-6 pt-10 pb-8">
        <div className="lilo-pipeline-hero mb-10">
          <div className="lilo-pipeline-hero-glow" />
          <div className="lilo-pipeline-hero-icon">
            <TitleIcon className="w-9 h-9" />
          </div>
          <div className="lilo-pipeline-hero-copy">
            <h1 className="lilo-pipeline-hero-title">{title}</h1>
            <p className="lilo-pipeline-hero-subtitle">{subtitle}</p>
          </div>
        </div>

        {pipeline === 'standard' && (
          <section className="mb-5 lilo-panel-card p-4">
              ...
              <div className="min-w-0 flex-1 overflow-x-auto">
                <div className="flex gap-3 pb-1">
                  {portraitTemplates.length ? (
                    ...
                    <div className="flex h-28 min-w-0 flex-1 items-center justify-center rounded-xl border border-dashed border-violet-200 bg-violet-50/40 text-xs text-gray-400">
                      暂无 1080x1920 模版
                    </div>
                  )}
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1.1fr)_minmax(340px,0.9fr)] gap-5 items-start">
          <section className="lilo-panel-card p-6">
            {pipeline === 'standard' && (
              <div className="space-y-4">
                    ...
                    <div className="grid h-10 max-w-sm grid-cols-3 rounded-lg bg-gray-100 p-1 text-sm">
                      {['9:16', '1:1', '16:9'].map(item => (
                        <button
                          ...
                          className={clsx('rounded-md transition-colors', ratio === item ? 'bg-white text-violet-600 shadow-sm' : 'text-gray-500')}
                        >
                          {item}
                        </button>
                      ))}
                      ...
                      {[
                        ...
                        <button
                          ...
                          className={clsx(
                            ...
                            templateMediaKind === item.id ? 'bg-white text-violet-600 shadow-sm' : 'text-gray-500',
                  ...
                  <div className="grid grid-cols-2 h-10 rounded-lg bg-gray-100 p-1 text-sm max-w-sm">
                    {[
                      ...
                      <button
                        ...
                        className={clsx('rounded-md transition-colors', standardVideoMode === item.id ? 'bg-white text-violet-600 shadow-sm' : 'text-gray-500')}
                      >
                        {item.label}
                      </button>
                    ))}
                ...
                <div className="lilo-tip-bar px-4 py-3">
                  <div className="mb-1 flex items-center gap-1.5 text-xs font-semibold lilo-tip-title">
                    <Lightbulb className="w-3.5 h-3.5" />
                    <span>说明</span>
                  </div>
                  <p className="text-sm leading-6 lilo-tip-text">
                    {templateMode
                ...
                <div className="grid grid-cols-2 h-10 rounded-lg bg-gray-100 p-1 text-sm max-w-sm">
                  {[
                    ...
                    <button
                      ...
                      className={clsx('rounded-md transition-colors', standardMode === item.id ? 'bg-white text-violet-600 shadow-sm' : 'text-gray-500')}
                    >
                      {item.label}
                    </button>
                  ))}
                ...
                <textarea
                  ...
                  className="w-full min-h-[150px] resize-none rounded-xl border border-violet-200/70 bg-white/80 px-3 py-3 text-sm text-gray-800 outline-none focus:border-violet-400 backdrop-blur-sm"
                />
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      {selectedTemplateFields.map(field => (
                        <label key={field.key} className="grid grid-cols-[92px_minmax(0,1fr)] items-center gap-2">
                          ...
                          <input
                            ...
                            className="h-9 min-w-0 rounded-lg border border-violet-200/70 bg-white/80 px-3 text-sm text-gray-700 outline-none focus:border-violet-400 backdrop-blur-sm"
                          />
                        </label>
                      ))}
                    </div>
            ...
            {pipeline === 'action_transfer' && (
              <div className="space-y-4">
                <textarea
                  ...
                  className="w-full min-h-[130px] resize-none rounded-xl border border-violet-200/70 bg-white/80 px-3 py-3 text-sm text-gray-800 outline-none focus:border-violet-400 backdrop-blur-sm"
                />
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            {pipeline === 'digital_human' && (
              <div className="space-y-4">
                ...
                <textarea
                  ...
                  placeholder="输入口播文案..."
                  className="w-full min-h-[130px] resize-none rounded-xl border border-gray-200 bg-white px-3 py-3 text-sm text-gray-800 outline-none focus:border-violet-300"
                />
              </div>
            )}
            ...
            <div className="mt-4 pt-4 border-t border-gray-100 flex flex-wrap items-center gap-2">
              <button
                ...
                className={clsx(
                  ...
                  showSettings ? 'bg-violet-50 text-violet-600' : 'text-gray-400 hover:text-gray-600 hover:bg-gray-50'
                )}
              >
              ...
              <button
                ...
                className={clsx(
                  ...
                  canSubmit && !running ? 'bg-violet-500 text-white hover:bg-violet-600 shadow-sm' : 'bg-gray-100 text-gray-400 cursor-not-allowed'
                )}
              >
              ...
              <div className="mt-4 p-4 bg-gray-50 rounded-xl space-y-3">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <label className="flex flex-col gap-1.5">
                    ...
                    <select value={ratio} onChange={e => setRatio(e.target.value)} className="h-10 rounded-lg border border-violet-200/70 bg-white/80 px-3 text-sm text-gray-700 outline-none backdrop-blur-sm">
                ...
                <label className="flex flex-col gap-1.5">
                  ...
                  <textarea
                    ...
                    className="w-full min-h-[70px] resize-none rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-700 outline-none focus:border-violet-300"
                  />
                </label>
                {pipeline === 'standard' && (
          ...
          <TaskResult task={task} onRetry={submit} />
        </div>
      </div>
