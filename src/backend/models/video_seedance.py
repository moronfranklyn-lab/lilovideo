...
class SeedanceVideoClient:
    ...
    def generate_video(
        ...
        **kwargs
        ...
        # 2. 轮询等待（顺带取回计费用量）
        video_url, usage = self._poll_until_done(task_id)

        # 3. 记账
        #
        # 为什么在这里记而不是在轮询里：轮询函数只认 task_id，拿不到 model，
        # 而定价是按 model 查的。
        #
        # **这里是整个成本链路最关键的一处。** Seedance 按 tokens 计费，
        # 而 tokens 只在任务响应的 usage 字段里，价目表读不出来。
        # 旧实现只 `return video_url`，把 usage 丢掉了——结果是
        # Seedance 视频（占全部成本约 80%）的成本永远算不出来。
        if usage:
            tokens = int(usage.get("completion_tokens") or usage.get("total_tokens") or 0)
            if tokens:
                report_usage(Usage(model_id=model, output_tokens=tokens))
                logger.info("SeedanceVideoClient: 计费用量 model=%s tokens=%s", model, tokens)
            else:
                logger.warning("SeedanceVideoClient: 任务成功但 usage 无 tokens: %s", usage)
        else:
            logger.warning(
                "SeedanceVideoClient: 任务成功但响应中没有 usage 字段，"
                "本次成本无法计算（费用不会凭空消失，只是我们看不到）：task_id=%s", task_id
            )
    ...
    def _poll_until_done(self, task_id: str, max_polls: int = 120, interval: int = 5) -> tuple[str, dict]:
        """轮询直到任务结束。

        返回 `(video_url, usage)`。**usage 必须原样带出去**——
        它是 Seedance 唯一的计费依据，丢在这里成本就永远算不出来。
        """
        # 同步更新查询接口路径
        ...
        for i in range(max_polls):
            ...
            if status == "succeeded":
                ...
                return video_url, (data.get("usage") or {})
